...
Titulo: Descifrado por medio de fuerza bruta de un texto cifrado con DES

Descripcion: El programa utiliza la clase Des_decrypter para generar y probar
llaves, con el fin de decifrar un texto.

Compilacion:
mpic++ -fopenmp des_alg_par.cpp -o des_alg_par -lstdc++

Correr:
mpirun -np N ./des_alg_par (donde N es el numero de procesos 1, 2, 4 o 6)

Para correr el programa es necesario tener en el mismo directorio un archivo
'cipher.txt' con el texto cifrado en bits.
------------------------------------------------------*/
#include <stdio.h>
#include <cstdio>
#include <iostream>
...
/* ---------------------------------------------------
La clase Des_decrypter esta basada en la implementacion del algoritmo
DES que se puede encontrar en este link: https://www.techiedelight.com/des-implementation-c/ .

Las funciones mas importantes aqui son create16Keys2(), decrypt() (tambien las funciones que se
llaman dentro de decrypt) y tryKey()
------------------------------------------------------*/
...
class Des_decrypter {
  public:
    ...
    std::string bittochar2()
    {
        std::string result;
        for (int i = 0; i < 64; i = i + 8) {

            result += convertToBits2(&ENCRYPTED[i]);
    ...
    // Funcion que recibe el texto cifrado en bits (plain[]) y tambien el largo del texto (n). Devuelve el texto descifrado en una string.
    std::string decrypt(long int n, long int plain[])
    {
        std::string res;
        for (int i = 0; i < n; i++)
        {
    ...
    // Esta funcion recibe la llave de 64 bits y la transforma segun el algoritmo DES
    void create16Keys2(unsigned int key[])
    {
        key64to48(key);
    }

    // recibe un entero y lo transforma en una llave de 64 bits
    unsigned int * generateKey(long int number){
      static unsigned int r[64];
      std::string binary = std::bitset<64>(number).to_string(); //to binary
      for (int itr = 0; itr<64; ++itr){
        r[itr] = (unsigned int)binary[itr]-48;

      }
      return r;
    }

    // Funcion que recibe una posible llave, el texto cifrado en bits y el largo del texto. Luego, descifra el texto cifrado utilizando la llave y busca una palabra clave que se encuentra dentro del texto, para verificar si la llave es la correcta.
    bool tryKey(long int inkey, long int plain[], long int n) // funcion que prueba si la llave es la correctoa
      {
        unsigned int *key;
        long int algo = inkey;
        key = generateKey(algo);
        create16Keys2(key);

        decrypt( n , plain );
        std::string rslt = decrypt( n , plain );
        std::size_t found = rslt.find("Octavious");
        if (found!=std::string::npos){
          return true;
          //std::cout << "La clave fue encontra: " << found << '\n';
        }else{
          return false;
        }
    }
...
// Devuelve el tamanio del texto cifrado. Este valor es utilizado por las funcion decrypt.
...
int main(int argc, char *argv[])
{
  ...
  // Se carga de 'cipher.txt' el texto cifrado.
  long int n = findFileSize() / 64;
  try{
  ...
  fclose(in);
  printf("Leyendo del archivo 'cipher.txt' el texto cifrado en bits\n");
  printf("Buscando la palabra 'Octavious' en el texto\n\n");

  int N, id;
  long upper = (1L <<56); //upper bound DES keys 2^56
  double start, end; //variables para medicion de tiempo
  // long upper = 10000;
  ...
  for(long i = mylower; i<myupper; ++i) // for que recorre el espacio de memoria de cada proceso
  {
    ...
    if(decrypter.tryKey(i, plain, n)) // if que determina si la llave es la correcta
    {
      found = i;
  ...
  if(id==0) // proceso rank 0 que espera hasta que se haya encontrado la llave y despliega el resultado
  {
    ...
    printf("La llave que descifra el texto es: %li\n", found);
