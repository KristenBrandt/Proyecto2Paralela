...
Titulo: Descifrado de un texto dado que sabemos la llave

Descripcion: El programa descifra el archivo 'cipher.txt' con la llave
que nosotros le pasamos. Esta debe cambiarse en la linea 709.

Compilacion:
gcc seq_decrypt.cpp -o seq_decrypt -lstdc++

Correr:
./seq_decrypt

Para correr el programa es necesario tener en el mismo directorio un archivo
'cipher.txt' con el texto cifrado en bits y haber introducido la llave correcta
en la linea 709.

Output:
un archivo 'result.txt' con el texto descifrado.
------------------------------------------------------*/
#include <stdio.h>
#include <cstdio>
#include <iostream>
...
class Des_decrypter {
  public:
    ...
    void decrypt(long int n, long int plain[])
    {
    ...
    // char search[] = " the ";
    // int tryKey(long int key, char *ciph, long int len) // funcion que prueba si la llave es la correctoa
    // {
    //   char temp[len+1];
...
int main()
{
    ...
    decrypter.out = fopen("result.txt", "wb+");
    fclose(decrypter.out);


    unsigned int *key;
    long int deciKey = 100000; // CAMBIAR AQUI la llave para descifrado
    key = decrypter.generateKey(deciKey);
    decrypter.create16Keys2(key);
